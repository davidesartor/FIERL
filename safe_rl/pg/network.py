...
def mlp_squashed_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space, policy_lower_bound, policy_upper_bound):
    """
    Experimental code for squashed gaussian policies, not yet tested
    """

    squashing_factor = (policy_upper_bound - policy_lower_bound) / 2
    shift_factor = (policy_upper_bound + policy_lower_bound) / 2
    ...
    pi = tf.tanh(u) * squashing_factor + shift_factor #modified, kl is invariant to affine transformation? 
...
def mlp_actor_critic(x, a, hidden_sizes=(64,64), activation=tf.tanh,
                     ...
                     squashed_policy:bool = False, policy_lower_bound:float = None, policy_upper_bound:float = None):

    # default policy builder depends on action space
    if policy is None and isinstance(action_space, Box):
        if squashed_policy:
            assert policy_lower_bound is not None and policy_upper_bound is not None
    ...
    with tf.variable_scope('pi'):
        if squashed_policy and policy == mlp_squashed_gaussian_policy:
            policy_outs = policy(x, a, hidden_sizes, activation, output_activation, action_space, policy_lower_bound, policy_upper_bound)
        else:
