...
class ObserverLogger: 
    ...
    def plot(self, save:bool = False, save_name:str = None, title_prefix:str = None):
        ...
        #plot red dashed line at the reference +- tracking threshold 
        # plt.plot(self.time_log, self.ref, '--', linewidth = 1, alpha=1, color="tab:red", label='reference')
        self.ref = np.array([[0.489],[0.2332]])
        # plt.plot(self.time_log, [self.ref[0] + 0.1]*len(self.time_log), '--', linewidth = 1., alpha=0.5, color="tab:red", label='threshold')
        # plt.plot(self.time_log, [self.ref[0] - 0.1]*len(self.time_log), '--', linewidth = 1., alpha=0.5, color="tab:red")
        plt.fill_between(self.time_log, self.ref[0] - 0.1, self.ref[0] + 0.1, alpha=0.1, color="tab:blue", label='$y_{0}^{ref} \pm \Delta y^{max}_{0}}$')
        plt.plot(self.time_log, [self.ref[0]]*len(self.time_log), '--', linewidth = 1., alpha=0.8, color="tab:blue",) # label='$y_{0}^{ref}$')
        # plt.plot(self.time_log, [self.ref[1] + 0.1]*len(self.time_log), '--', linewidth = 1., alpha=1, color="tab:orange", label='threshold')
        # plt.plot(self.time_log, [self.ref[1] - 0.1]*len(self.time_log), '--', linewidth = 1., alpha=1, color="tab:orange")
        plt.fill_between(self.time_log, self.ref[1] - 0.1, self.ref[1] + 0.1, alpha=0.1, color="tab:orange", label='$y_{1}^{ref} \pm \Delta y^{max}_{1}}$')
        plt.plot(self.time_log, [self.ref[1]]*len(self.time_log), '--', linewidth = 1., alpha=0.8, color="tab:orange", )
        plt.xlim(left=0, right=12)
        plt.ylim(bottom = 0.0)
        plt.legend(loc='bottom right')
        plt.ylabel('system output (m)')
        plt.plot()
...
def plot_comparison1(ObserverLogger1:ObserverLogger, ObserverLogger2:ObserverLogger, ObserverLogger3:ObserverLogger, data_type:str, labels:list = None, save:bool = False, save_name:str = None) -> None: 
    """
    Plot n plots, where n is the dimension of the data_type. i-th plot displays the i-th component 
    of the data_type of both ObserverLogger1 and ObserverLogger2.
    ...
    Args: 
        ...
        ObserverLogger3 (ObseverLogger): 
            third observer logger
    ...
    if data_type == 'state': 
        ...
        data1 = ObserverLogger1.mean_state_log
        data2 = ObserverLogger2.mean_state_log
        data3 = ObserverLogger3.mean_state_log
        corr1 = ObserverLogger1.cov_state_log
        corr2 = ObserverLogger2.cov_state_log
        corr3 = ObserverLogger3.cov_state_log
        title = 'State Estimate Evolution'
    elif data_type == 'fault': 
        true_data = ObserverLogger1.fault_log
        data1 = ObserverLogger1.mean_fault_log
        data2 = ObserverLogger2.mean_fault_log
        data3 = ObserverLogger3.mean_fault_log
        corr1 = ObserverLogger1.cov_fault_log
        corr2 = ObserverLogger2.cov_fault_log
        corr3 = ObserverLogger3.cov_fault_log
        title = 'Fault Estimate Evolution'
    elif data_type == 'output': 
        data1 = ObserverLogger1.output_log
        data2 = ObserverLogger2.output_log
        data3 = ObserverLogger3.output_log
        title = 'System Output Envolution'
    elif data_type == 'control_input': 
        data1 = ObserverLogger1.control_input_log
        data2 = ObserverLogger2.control_input_log
        data3 = ObserverLogger3.control_input_log
        title = 'Policy Evolution'
    else: 
        raise ValueError(f"Unknown data type {data_type}. Must be 'state', 'fault', 'output' or 'control_input'")
    ...
    if data_type == 'state' or data_type == 'fault': 
        ...
        autocorr1 = np.array([np.sqrt(np.diag(cov)) for cov in corr1]).T
        autocorr2 = np.array([np.sqrt(np.diag(cov)) for cov in corr2]).T
        autocorr3 = np.array([np.sqrt(np.diag(cov)) for cov in corr3]).T
    
    data1 = np.hstack([x.reshape(-1,1) for x in data1])
    data2 = np.hstack([x.reshape(-1,1) for x in data2])
    data3 = np.hstack([x.reshape(-1,1) for x in data3])

 
    bias = 0.0001

    for i,(data1_c,data2_c,data3_c) in enumerate(zip(data1,data2,data3)): 
        plt.figure(figsize=(6, 3.25))
        if data_type == 'state' or data_type == 'fault': 
            if data_type == 'state': 
                prefix = 'State_estimate_'
            ...
            if data_type == 'fault': 
                prefix = 'Fault_estimate_'
                # labs = [f'$z^{{true}}_{i}$', f'$z^{{est}}_{i}$' + labels[0] , f'$z^{{est}}_{i}$' + labels[1]]
                labs = [f'$z^{{true}}_{i}$', f'$\mu^{{est}}_{{z_{i}}} \pm \sigma^{{est}}_{{z_{i}}} $' + labels[0] , f'$\mu^{{est}}_{{z_{i}}} \pm \sigma^{{est}}_{{z_{i}}} $' + labels[1], f'$\mu^{{est}}_{{z_{i}}} \pm \sigma^{{est}}_{{z_{i}}} $' + labels[2]]
            plt.plot(ObserverLogger1.time_log, true_data[i,:], '-', linewidth = 1, alpha=0.5, color='k', label=labs[0])
            
            plt.plot(ObserverLogger1.time_log, data1_c, '-', linewidth = 1.5, alpha=1, color=colors[i], )
            plt.fill_between(ObserverLogger1.time_log, data1_c-np.array(autocorr1[i]-bias), data1_c+np.array(autocorr1[i]-bias), alpha=0.3, color=colors[i], label=labs[1])

            # plt.plot(ObserverLogger2.time_log, data2_c, '--', linewidth = 1, alpha=0.5, color="gray", )
            # plt.fill_between(ObserverLogger2.time_log, data2_c-np.array(autocorr2[i]-bias), data2_c+np.array(autocorr2[i]-bias), alpha=0.2, edgecolor="gray", hatch="//", facecolor="none", label=labs[2])

            plt.plot(ObserverLogger3.time_log, data3_c, '--', linewidth = 1, alpha=1, color="gray",)           
            plt.fill_between(ObserverLogger3.time_log, data3_c-np.array(autocorr3[i]-bias), data3_c+np.array(autocorr3[i]-bias), alpha=0.1, color="gray", label=labs[3])
            plt.ylabel('fault estimate of component ' +  f' {i}')
            ...
            plt.step(ObserverLogger1.time_log, data1_c, '-', linewidth = 1.5, alpha=1, color=colors[i], label=labs[0])
            plt.step(ObserverLogger2.time_log, data2_c, '-', linewidth = 1, alpha=1, color="tab:gray", label=labs[1])
            plt.step(ObserverLogger3.time_log, data3_c, '-', linewidth = 1, alpha=1, color="tab:gray", label=labs[2])
            ...
            plt.step(ObserverLogger1.time_log[1:], data1_c, '-', linewidth = 1.5, alpha=1, color=colors[i], label=labs[0])
            plt.step(ObserverLogger2.time_log[1:], data2_c, '-', linewidth = 1, alpha=1, color="tab:gray", label=labs[1])
            plt.step(ObserverLogger3.time_log[1:], data3_c, '-', linewidth = 1, alpha=1, color="tab:gray", label=labs[2])
