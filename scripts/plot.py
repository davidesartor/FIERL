...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", 
              ...
              hidelegend=True, title=None, savedir=None, 
              clear_xticks=False, color="tab:blue", **kwargs):
 ...
 (0.09019607843137255, 0.7450980392156863, 0.8117647058823529)]
    ...
    #"""
    sns.tsplot(data=data, time=xaxis, value=value, unit="Unit", condition=condition, ci='sd', color=color, **kwargs)
    """
...
def get_datasets(logdir, condition=None):
    ...
    for root, _, files in os.walk(logdir):
        if 'progress.txt' in files:
            ...
            cost_performance = 'AverageEpCost'
            exp_data.insert(len(exp_data.columns),'Unit',unit)
            exp_data.insert(len(exp_data.columns),'Condition1',condition1)
            exp_data.insert(len(exp_data.columns),'Condition2',condition2)
            exp_data.insert(len(exp_data.columns),'Performance',exp_data[performance])
            exp_data.insert(len(exp_data.columns), 'CostPerformance', exp_data[cost_performance])
...
def make_plots(all_logdirs, legend=None, xaxis=None, values=None, count=False,  
               ...
               paper=False, hidelegend=False, title=None, savedir=None, show=True, color='tab:blue',
               clear_xticks=False):
...
def main():
    ...
    make_plots(args.logdir, args.legend, args.xaxis, args.value, args.count, 
               ...
               savedir=args.savedir, show=not(args.dont_show),
               clear_xticks=args.clearx, title = "Average Return")
    make_plots(args.logdir, args.legend, args.xaxis, "CostPerformance", args.count, 
               smooth=args.smooth, select=args.select, exclude=args.exclude,
               estimator=args.est, paper=args.paper, hidelegend=args.hidelegend,
               savedir=args.savedir, show=not(args.dont_show),
               clear_xticks=args.clearx, color = "tab:red", title = "Average C-Return")
    make_plots(args.logdir, args.legend, args.xaxis, "CostRate", args.count, 
               smooth=args.smooth, select=args.select, exclude=args.exclude,
               estimator=args.est, paper=args.paper, hidelegend=args.hidelegend,
               savedir=args.savedir, show=not(args.dont_show),
               clear_xticks=args.clearx, color = "tab:orange", title = "Average Cost Constraint Violation Rate")
