...
def main(args, cfg_env = None, training_config = None): 
    ...
    fault_observer = FaultObserver(state_dim = system.state_dim, input_dim = system.input_dim, fault_evol_cov = cfg_env['fault_evol_cov'])
    ic_sampling = InitialConditionSampler(ic_type = cfg_env['ic_type'], half_side = cfg_env['half_side'], center = cfg_env['center'], n_samples=1)
    fault_sampling = FaultSampler(fault_type = cfg_env['fault_type'], a = cfg_env['a'], b = cfg_env['b'], size = (system.input_dim, ))
    env = Environment(system=system, 
                      fault_observer = fault_observer, 
                      reference = cfg_env['reference'],
                      track_threshold = cfg_env['tracking_threshold'],
                      initial_condition_fnc = ic_sampling, 
                      fault_generator_fnc = fault_sampling, 
